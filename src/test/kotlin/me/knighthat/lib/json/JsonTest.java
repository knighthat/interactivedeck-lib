...
class JsonTest {
    ...
    private static final byte[] compressedBytes = new byte[]{ 31, -117, 8, 0, 0, 0, 0, 0, 0, -1, -19, 87, 91, 111, -37, 54, 20, 126, -49, -81, 16, -4, 28, 90, -30, 77, -105, -66, -91, -119, -117, 5,
    ...
    private static JsonArray compressed;

    @BeforeAll
    static void setUp() {
        ClassLoader loader = JsonTest.class.getClassLoader();

        try (InputStream inStream = loader.getResourceAsStream( "test.profile" ) ;
             Reader reader = new InputStreamReader( inStream )) {

            candidate = JsonParser.parseReader( reader );
            compressed = Json.gzipCompress( candidate );
    ...
    @Test
    void isGzip() {
        Assertions.assertTrue( Json.isGzip( compressed ) );
    }

    @Test
    void gzipCompress() {
        Assertions.assertArrayEquals( compressedBytes, JsonArrayConverter.toByteArray( compressed ) );
    }

    @Test
    void gzipDecompress() {
        try {
            JsonElement inflated = Json.gzipDecompress( compressed );
            Assertions.assertEquals( candidate, inflated );
        } catch (IOException e) {
